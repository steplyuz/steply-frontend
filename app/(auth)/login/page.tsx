  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.status !== "success") {
        fail(res.message || "Kirishda xatolik yuz berdi")
        return
      }
      ...
      fail(err instanceof ApiRequestError ? err.message : "Kirishda xatolik yuz berdi")
    }
  }

  return (
    <form onSubmit={handleSubmit} noValidate className="w-full">
      <div ref={fieldRef} className="space-y-3.5">
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              required
              disabled={loading}
              className="h-[52px] w-full rounded-xl border border-[#DCDDE2] bg-white pl-14 pr-4 text-[15px] font-medium text-[#0B0B0B]
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="h-[52px] w-full rounded-xl border border-[#DCDDE2] bg-white pl-14 pr-12 text-[15px] font-medium text-[#0B0B0B]
                         outline-none transition-[border-color,box-shadow] placeholder:text-[#B0B1B9]
                         focus:border-[#0B0B0B] focus:ring-4 focus:ring-[#E4F60A]/60 disabled:opacity-60"
            />
            <button
      ...
      <div className="mt-3 flex justify-end">
        <Link href="/forgot-password" className="text-xs font-semibold text-[#8A8C94] hover:text-[#0B0B0B]">
          Parolni unutdingizmi?
        </Link>
      </div>

      {error && (
        <p id="login-error" role="alert" className="mt-3 flex items-start gap-2 text-sm text-red-600">
      ...
      <p className="mt-5 text-center text-sm text-[#8A8C94]">
        Hisobingiz yo'qmi?{" "}
        <Link href="/register" className="font-semibold text-[#0B0B0B] hover:underline">
          Ro'yxatdan o'ting
        </Link>
      </p>
    </form>
      ...
      >
        ...
        <div ref={headlineRef} className="hidden items-center self-stretch lg:flex lg:pl-[8vw] xl:pl-[10vw]">
          <h1 className="text-[clamp(2.4rem,3.6vw,3.8rem)] font-extrabold leading-[1.1] tracking-[-0.03em]">
            O&lsquo;qishingizni
            <br />
            <span style={{ color: LIME_TEXT }}>bir joydan boshqaring</span>
          </h1>
        </div>
