...
type ResultPayload = VerifiedResult & {
...
const HOW_STEPS = [
    {
        ...
        body: "Natija qaydnomasidagi QR tokenni quyidagi maydonga kiriting.",
    },
    {
        icon: ExternalLink,
        title: "Natija yuklanadi",
        body: "Bir zumda barcha 4 ko'nikma bo'yicha ballaringiz ko'rinadi.",
    },
    {
        icon: BadgeCheck,
        title: "CEFR darajangiz",
        body: "Umumiy ball va CEFR darajangizni tasdiqlangan holda ko'ring.",
    },
]
    ...
    return (
        ...
        >
            ...
            {[
                ...
                <span
                    key={cls}
                    className={`absolute size-3 rounded-[3px] border-foreground/70 ${cls}`}
                />
            ))}
        </div>
    )
        ...
        >
            ...
            >
                ...
                <p className="mt-4 rounded-xl bg-muted/60 p-3 text-xs leading-5 text-muted-foreground">
                    Bu blok faqat interfeys namunasi — haqiqiy natijani quyidagi forma
                    orqali tekshiring.
                </p>
            </motion.div>
        </motion.div>
...
function SearchForm({
    ...
    onResult: (result: ResultPayload | null) => void
}) {
    const id = useId()
    ...
    async function submit(e: FormEvent) {
        ...
        try {
            const response = await publicApi.resultByQr(cleanToken)
            onResult(response)
        } catch (e) {
...
function ResultCard({
    ...
    const overall = Number(
        result.overall_score ?? result.overall ?? 0
    )
    ...
    function skillScore(skill: VerifiedResultSkill | undefined) {
        return Number(skill?.scaled_score ?? 0)
    }

    const clampedOverall = Math.max(0, Math.min(90, overall))
            ...
            <div className="relative overflow-hidden rounded-2xl border border-border/80 bg-card shadow-[0_12px_40px_-20px_rgba(0,0,0,0.18)] sm:rounded-3xl">
                ...
                >
                    {SKILLS.map(({ label, key, short }) => {
                        const score = skillScore(result[key])
                        ...
                        return (
                            ...
                            >
                                ...
                                <p
                                    className={`mt-2 text-2xl font-bold tabular-nums ${text}`}
                                >
                                    {score.toFixed(1)}
                                </p>
                ...
                <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-border px-5 py-4">
                    ...
                    {result.exam_date && (
                        <p className="text-sm text-muted-foreground">
                            {new Date(String(result.exam_date)).toLocaleDateString(
                                "uz-UZ",
                                { dateStyle: "long" }
                            )}
                        </p>
                            ...
                            <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        bo&apos;yicha batafsil natijangiz bir zumda
                                        ko&apos;rinadi.
                                    </motion.p>
                                ...
                                {[
                                    {
                                        n: "01",
                                        title: "QR token nima?",
                                        desc: "Har bir mock imtihon qaydnomasida berilgan noyob identifikator.",
                                    },
                                    {
                                        n: "02",
                                        title: "Natijalar qachon?",
                                        desc: "Imtihon sanasidan 3–5 ish kuni ichida portal orqali e'lon qilinadi.",
                                    },
                                    {
                                        n: "03",
                                        title: "Shaxsiy kabinet",
                                        desc: "Ro'yxatdan o'tgan foydalanuvchilar barcha natijalarini kabinetda ko'radi.",
                                    },
                                ].map((item) => (
                                    <div
                                        key={item.n}
